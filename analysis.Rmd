...
```{r}
wind_csv = read_csv("uswtdbCSV/uswtdb_v1_2_20181001.csv")
wind_csv
```

Analysis - group by state, graph height (and get none NA vals), what heights
```{r}
wind_csv %>% select(case_id,
                    state = t_state,
                    county = t_county,
                    site_name = p_name,
                    site_year = p_year,
...
[DONE] Use of spatial vector data (sf package) and visualization of spatial data
